...
class Task(models.Model):
    ...
    status = models.CharField(
        ...
        default=TaskStatus.SENT
    )
    is_active = models.BooleanField(_('Активная'), default=True)
    # Single assignment fields are deprecated but kept for backward compatibility
    # New system uses TaskClientAssignment and TaskEmployeeAssignment models
    assigned_to = models.ForeignKey(
        ...
        verbose_name=_('Назначено сотруднику (устаревшее)')
    )
    client = models.ForeignKey(
        ...
        blank=True,
        related_name='tasks',
        verbose_name=_('Клиент (устаревшее)')
    )
    ...
    client_group = models.ForeignKey(
        ...
        help_text=_('Группа клиентов для назначения задачи')
    )
    employee_group = models.ForeignKey(
        ...
        null=True,
        blank=True,
        verbose_name=_('Группа сотрудников'),
        help_text=_('Группа сотрудников для назначения задачи')
    )
    # Поле для хранения количества стендов в задаче
    stand_count = models.PositiveIntegerField(
        _('Количество стендов по умолчанию'),
        default=0,
        help_text=_('Количество стендов, указанное при создании задачи')
    )
    ...
    def can_be_viewed_by(self, user):
        ...
        if user.role == UserRoles.EMPLOYEE:
            ...
            is_assigned = (
                ...
                self.assigned_to is None or
                self.is_employee_assigned(user.id)
            )
            return (
    ...
    def get_completion_percentage(self):
        """Возвращает процент завершения для задач типа анкеты."""
        if self.task_type != TaskType.SURVEY:
            return 0
            
        if self.target_count > 0:
            # Явно преобразуем в float для корректного деления
            return int((float(self.current_count) / float(self.target_count)) * 100)
        return 0
        
    def get_assigned_clients(self):
        """Возвращает список клиентов, назначенных на задачу."""
        from clients.models import Client
        # Check if task has client assignments
        if self.taskclientassignment_set.exists():
            return list(self.taskclientassignment_set.select_related('client').values_list('client_id', flat=True))
        # Fallback to old single client assignment
        elif self.client:
            return [self.client.id]
        # Check if task has client group
        elif self.client_group:
            return list(self.client_group.client_set.values_list('id', flat=True))
        # If no specific assignment, return all clients (backward compatibility)
        else:
            return list(Client.objects.values_list('id', flat=True))
    
    def get_assigned_employees(self):
        """Возвращает список сотрудников, назначенных на задачу."""
        from users.models import CustomUser
        # Check if task has employee assignments
        if self.taskemployeeassignment_set.exists():
            return list(self.taskemployeeassignment_set.select_related('employee').values_list('employee_id', flat=True))
        # Fallback to old single employee assignment
        elif self.assigned_to:
            return [self.assigned_to.id]
        # Check if task has employee group
        elif self.employee_group:
            return list(self.employee_group.customuser_set.filter(role='EMPLOYEE').values_list('id', flat=True))
        # If no specific assignment, return all employees
        else:
            return list(CustomUser.objects.filter(role='EMPLOYEE').values_list('id', flat=True))
    
    def is_client_assigned(self, client_id):
        """Проверяет, назначен ли клиент на задачу."""
        # Преобразуем client_id в int, если он строка
        ...
        # Проверяем через get_assigned_clients
        assigned_clients = self.get_assigned_clients()
        return client_id in assigned_clients
    
    def all_clients_submitted(self):
        """Проверяет, отправили ли все назначенные клиенты фотоотчёты."""
        from .models import PhotoReport
        if self.task_type not in [TaskType.EQUIPMENT_PHOTO, TaskType.SIMPLE_PHOTO]:
            return False
            
        assigned_clients = set(self.get_assigned_clients())
        if not assigned_clients:
            # Если нет назначенных клиентов, считаем, что задача выполнена
            return True
            
        submitted_clients = set(
            PhotoReport.objects.filter(task=self).values_list('client_id', flat=True)
        )
        return assigned_clients == submitted_clients
    
    def get_photo_completion_percentage(self):
        """Возвращает процент завершения для задач типа фотоотчёта."""
        if self.task_type not in [TaskType.EQUIPMENT_PHOTO, TaskType.SIMPLE_PHOTO]:
            return 0
            
        from .models import PhotoReport
        total_clients = len(self.get_assigned_clients())
        if total_clients == 0:
            return 100 # Если нет клиентов, считаем задачу выполненной
        
        submitted_clients = PhotoReport.objects.filter(task=self).values_list('client_id', flat=True).distinct().count()
        return int((submitted_clients / total_clients) * 100)
    
    def is_employee_assigned(self, employee_id):
        """Проверяет, назначен ли сотрудник на задачу."""
        return employee_id in self.get_assigned_employees()
    
    def get_target_client_count(self):
        """Возвращает количество целевых клиентов для статистики."""
        return len(self.get_assigned_clients())
    
    def get_completed_client_ids_for_user(self, user_id):
        """Возвращает список ID клиентов, по которым пользователь уже выполнил задачу."""
        if self.task_type == TaskType.SURVEY:
        ...
        elif self.task_type in [TaskType.EQUIPMENT_PHOTO, TaskType.SIMPLE_PHOTO]:
            from .models import PhotoReport
            return PhotoReport.objects.filter(
                task=self,
...
class PhotoReport(models.Model):
    ...
    # Оценки по категориям (1-3)
    equipment_fill = models.PositiveSmallIntegerField(
        _('Наполнение оборудования'),
        choices=[(1, 'Плохо'), (2, 'Средне'), (3, 'Хорошо')],
        null=True,
        blank=True,
        help_text=_('Оценка наполнения оборудования: 1 — Плохо, 2 — Средне, 3 — Хорошо')
    )
    no_foreign_goods = models.PositiveSmallIntegerField(
        _('Отсутствие посторонних товаров'),
        choices=[(1, 'Плохо'), (2, 'Средне'), (3, 'Хорошо')],
        null=True,
        blank=True,
        help_text=_('Оценка отсутствия посторонних товаров на оборудовании')
    )
    product_display = models.PositiveSmallIntegerField(
        _('Выкладка и оформление товаров'),
        choices=[(1, 'Плохо'), (2, 'Средне'), (3, 'Хорошо')],
        null=True,
        blank=True,
        help_text=_('Оценка выкладки и оформления торговых материалов')
    )
    equipment_location = models.PositiveSmallIntegerField(
        _('Расположение оборудования в торговой точке'),
        choices=[(1, 'Плохо'), (2, 'Средне'), (3, 'Хорошо')],
        null=True,
        blank=True,
        help_text=_('Оценка расположения оборудования в торговом зале')
    )
    ...
    moderated_at = models.DateTimeField(
        ...
        null=True,
        blank=True,
        help_text=_('Время, когда модератор обработал отчёт')
    )
    
    def __str__(self):
        report_type = _("оборудованию") if self.is_equipment_report() else _("простой")
        status = " (Новый)" if self.moderated_at is None else f" (Обработано {self.moderated_by.username if self.moderated_by else 'модератором'})"
        return f"Фотоотчет ({report_type}) для {self.client.name}{status}"
    ...
    def get_average_rating(self):
        """
        Возвращает средний рейтинг по четырём категориям:
        - наполнение оборудования,
        - отсутствие посторонних товаров,
        - выкладка и оформление товаров,
        - расположение оборудования.
        
        Оценки принимают значения от 1 (Плохо) до 3 (Хорошо).
        Если хотя бы одна оценка отсутствует — возвращает None.
        """
        ratings = [
            self.equipment_fill,
            self.no_foreign_goods,
    ...
    def is_fresh(self):
        """
        Проверяет, является ли отчёт 'свежим':
        - не обработан модератором,
        - и текущее время < 18:00 по Красноярскому времени (UTC+7)
          следующего дня после создания отчёта.
          
        Используется для визуального выделения новых отчётов в интерфейсе.
        """
        from datetime import datetime, time, timedelta
        import pytz

        # Определяем, обработан ли отчёт
        if self.moderated_at is not None:
            return False

        # Красноярское время: UTC+7
        krsk_tz = pytz.timezone('Asia/Krasnoyarsk')
        # Приводим время создания к Красноярску
        created_krsk = self.created_at.astimezone(krsk_tz)

        # Следующий день после создания
        next_day = created_krsk.date() + timedelta(days=1)
        # Дедлайн: 18:00 следующего дня
        deadline_krsk = krsk_tz.localize(datetime.combine(next_day, time(18, 0, 0)))

        now_krsk = datetime.now(krsk_tz)
        return now_krsk < deadline_krsk


class EquipmentPhotoReport(PhotoReport):
    """
    Прокси-модель для фотоотчетов по оборудованию.
    """
    class Meta:
        proxy = True
        verbose_name = _('Фотоотчет по оборудованию')
        verbose_name_plural = _('Фотоотчеты по оборудованию')
...
class PhotoReportItem(models.Model):
    ...
    latitude = models.FloatField(
        ...
        null=True,
        blank=True,
        help_text=_('Географическая широта места съемки фото')
    )
    longitude = models.FloatField(
        _('Долгота'),
        null=True,
        blank=True,
        help_text=_('Географическая долгота места съемки фото')
    )
    location_address = models.CharField(
        _('Адрес местоположения'),
        max_length=500,
        blank=True,
        null=True,
        help_text=_('Адрес, полученный по координатам (обратное геокодирование)')
    )
...
# Add this before the existing TaskSurveyStatsProxy class

class TaskClientAssignment(models.Model):
    """
    Связь между задачей и клиентом для поддержки множественного назначения.
    ...
    class Meta:
        verbose_name = _('Назначение клиента к задаче')
        verbose_name_plural = _('Назначения клиентов к задачам')
        unique_together = ('task', 'client')


class TaskEmployeeAssignment(models.Model):
    """
    Связь между задачей и сотрудником для поддержки множественного назначения.
    ...
    employee : CustomUser
        Сотрудник
    completed : bool
        Завершено ли задание для этого сотрудника
    completed_at : datetime, optional
        Время завершения
    created_at : datetime
        Время создания
    ...
    class Meta:
        verbose_name = _('Назначение сотрудника к задаче')
        verbose_name_plural = _('Назначения сотрудников к задачам')
        unique_together = ('task', 'employee')


class TaskSurveyStatsProxy(Task):
    class Meta:
        proxy = True
        verbose_name = _('Статистика задачи')
...
class DailyTask(models.Model):
    """
    Модель задачи дня для торгового представителя.
    ...
    status : str
        Статус задачи (активная, завершенная)
    priority : int
        Приоритет задачи (1-5)
    position : int
        Позиция задачи в списке (для сортировки)
    created_at : datetime
        Время создания
    updated_at : datetime
        Время последнего обновления
    ...
    date = models.DateField(_('Дата задачи'))
    client = models.ForeignKey(
        'clients.Client',
        on_delete=models.CASCADE,
        verbose_name=_('Клиент')
    ...
    result = models.TextField(_('Результат выполнения задачи'), blank=True, null=True)
    status = models.CharField(
        _('Статус задачи'),
        max_length=20,
    ...
    def __str__(self):
        return f"Задача дня: {self.creator.username} -> {self.assignee.username} ({self.date})"

...
class EmployeeGroup(models.Model):
    """
    Employee group model for organizing employees.
    ...
    name = models.CharField(_('Название группы сотрудников'), max_length=100)
    description = models.TextField(_('Описание'), blank=True, null=True)
    created_at = models.DateTimeField(_('Создано'), auto_now_add=True)
    updated_at = models.DateTimeField(_('Обновлено'), auto_now=True)
    
    def __str__(self):
        """Return string representation of employee group."""
        return self.name
    
    class Meta:
        verbose_name = _('Группа сотрудников')
        verbose_name_plural = _('Группы сотрудников')
...
class CustomUser(AbstractUser):
    ...
    employee_groups = models.ManyToManyField(
        EmployeeGroup,
        blank=True,
        verbose_name=_('Группы сотрудников')
    )

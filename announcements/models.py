...
class Announcement(models.Model):
    ...
    def acknowledged_count(self):
        """Returns the number of users who have acknowledged reading the announcement."""
        return self.read_statuses.filter(acknowledged=True).count()
    acknowledged_count.short_description = 'Подтверждено'

    def acknowledged_ratio(self):
        """Returns a string 'X out of Y' showing how many recipients acknowledged the announcement."""
        acknowledged = self.read_statuses.filter(acknowledged=True).count()
        total_recipients = self.recipients.count()
        return f"{acknowledged} из {total_recipients}"
    acknowledged_ratio.short_description = 'Подтверждено'

...
class PhotoReportView(LoginRequiredMixin, FormView):
    ...
    def form_valid(self, form):
        ...
        # Update task status
        task.status = TaskStatus.ON_CHECK
        task.is_active = False  # Photo report becomes inactive after submission
        task.save()
...
def autocomplete_clients(request):
    ...
    # Case-insensitive search using __icontains (more reliable than iregex)
    clients = Client.objects.filter(
        name__icontains=query
    ).order_by('name')[:20]
...
def equipment_photo_reports_api(request):
    ...
    for report in reports:
        report_data = {
            ...
            'userName': report.created_by.get_full_name() or report.created_by.username,
...
def send_photo_report_to_rework(request):
    ...
    try:
        ...
        # Обновляем комментарий модератора и статус
        ...
        return JsonResponse({
            'success': True,
            'message': 'Отчёт отправлен на доработку',
            'status': 'REWORK'
        })
...
def process_equipment_photo_report(request):
    ...
    try:
        ...
        report.save(update_fields=rating_fields + ['moderated_by', 'moderated_at'])
        return JsonResponse({
            'success': True,
...
def accept_equipment_photo_report(request):
    ...
    Обновляет статус задачи на ACCEPTED.
    """
    try:
        data = json.loads(request.body)
        report_id = data.get('report_id')
